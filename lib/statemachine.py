...
class Statemachine:
    ...
    def tick(self):
        if self.last_in_state_action + self.STATE_ACTION_DELAY_MS < round(
            time.monotonic() * 1000
        ):
            self.do_state_action()
            self.last_in_state_action = round(time.monotonic() * 1000)
        if self.last_state_transition + self.MIN_STATE_DELAY_MS < round(
            time.monotonic() * 1000
        ):
    ...
    def check_if_pyro_should_be_fired(self):
        if (
            self.state != States.LAUNCHED_MODE
        ):  # Not in launching state of the statediagram
            return False
        if self.LAUNCHED == False:  # Has not launched yet
            return False
        if self.launched_time == 13:  # The launchtimer has not been set yet
            return False
        if self.launched_time + self.PYRO_FIRE_DELAY_MS < round(
            time.monotonic() * 1000
    ...
    def _battery_voltage_valid(
        self,
    ):  # Todo: Make it depend on the battery configuration
